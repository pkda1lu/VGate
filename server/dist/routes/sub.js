...
exports.default = subRoutes;
const db_1 = require("../db");
const schema_1 = require("../db/schema");
const drizzle_orm_1 = require("drizzle-orm");
const link_generator_1 = require("../utils/link-generator");
const SettingsService_1 = require("../services/SettingsService");
async function subRoutes(fastify, options) {
    ...
    fastify.get('/:subId', async (request, reply) => {
        ...
        const clients = await db_1.db.query.clients.findMany({
            where: (0, drizzle_orm_1.eq)(schema_1.clients.subId, subId),
            with: {
                inbound: {
                    with: {
                        node: true
        ...
        const masterIp = await settingsService.getSetting('server_ip', '127.0.0.1');
        const panelDomain = await settingsService.getSetting('panel_domain', masterIp);
        const subPort = await settingsService.getSetting('sub_port', '4000');
        const subPath = await settingsService.getSetting('sub_path', '/api/sub/');
        const links = clients.map(client => {
            ...
            if (host === 'localhost' || host === '127.0.0.1')
                host = masterIp;
            return (0, link_generator_1.generateProxyUri)(inbound, client, host);
        }).filter(Boolean);
